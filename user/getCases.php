<?php
    ...
    $sql =    "SELECT user, date_visit, poi_id
                ...
                WHERE user != '".$_SESSION["user_id"]."' AND (date_visit >= CURDATE() - INTERVAL 7 DAY)  AND
                        EXISTS(SELECT * FROM cases WHERE (case_date BETWEEN v.date_visit AND v.date_visit + INTERVAL 7 DAY) and user = v.user) ORDER BY date_visit DESC";
    ...
    while($row = $result->fetch_assoc()){
        $sql = "SELECT name, adress,date_visit
                ...
                AND user = '".$_SESSION["user_id"]."' ORDER BY date_visit DESC";
